...
for (plot_name in names(all_plots)) {
  filename <- paste0("figures/climatology/", tolower(gsub(" ", "_", plot_name)), ".jpg")
...
# library(climate)
# 
# # Assuming state_data_list is a list of climate data for each state
# # Replace state_index_number with the desired state's index
# #state_index_number <- c(6,10,16,24,31,33)  # Change this to the desired state's index
# 
# state_index_number <- 6
# # Function to create and save climatology plots
# create_climatology_plots <- function(state_data, state_name, years, met_var) {
#   # Create the climatology plot
#   climatology_plot <- plot(
#     state_data,
#     years = years,
...
#     theme_minimal() +
#     labs(
#       title = paste("Climatology Plot -", state_name, "-", met_var),
#       x = "Month",
#       y = "Cumulative Value"
#     )
#   
#   # Save the plot
#   filename <- paste0("figures/climatology/", tolower(gsub(" ", "_", state_name)), "_", met_var, "_climatology.jpg")
#   ggsave(filename, climatology_plot, width = 8, height = 6, units = "in")
# }
# 
# # Loop through the selected years and create plots for rainfall and temperature
# for (start_year in c(1990, 2001, 2011)) {
#   end_year <- start_year + 9
#   selected_years <- start_year:end_year
#   
#   # Plot Rainfall
#   create_climatology_plots(
#     state_data_list[[state_index_number]],
#     state_coordinates$State[state_index_number],
#     selected_years,
#     "rain"
#   )
#   
...
#   
#   # Save the plots
#   all_plots[[paste("Rainfall_", start_year, "-", end_year)]] <- rainfall_plot
#   all_plots[[paste("Max_Temperature_", start_year, "-", end_year)]] <- max_temp_plot
#   all_plots[[paste("Min_Temperature_", start_year, "-", end_year)]] <- min_temp_plot
# }
# 
# # Save all plots
# for (plot_name in names(all_plots)) {
